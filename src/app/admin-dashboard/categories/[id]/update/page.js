...
const UpdateCategoryPage = () => {
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      try {
        const token = user?.token;
        const prevProducts = categoryData.products.map(
          (item) => item.product_id
        );
        const currentProducts = formData.products.map(
          (item) => item.product_id
        );
        ...
        await updateCategory(params.id, data, token);
    ...
    if (params.id) {
      const fetchCategory = async () => {
        try {
          ...
          // Call the API service function with pagination parameters
          const response = await getCategoryById(params.id, token);
          console.log("Category API Response:", response);

          // Check for API errors using hasError property
          if (response?.hasError) {
            const errorMessage =
              response.message || "Failed to load category. Please try again.";
    ...
    if (categoryData) {
      setFormData((prev) => {
        return {
          ...
          products: categoryData.products.map((item) => {
            return {
